...
access_granted = "🔓 Доступ к ПУ предоставлен!\n\n" \
                 ...
                 "⚙️ Чтобы открыть меню настроек <i>Playerok Cardinal</i>, введи команду /menu."
...
✅ <i>POC</i> может точно определить, кто именно написал, будь то ты, твой собеседник или 3е лицо (арбитраж).
❌ Из-за того что <i>POC</i> получает полную историю чата для обнаружения новых сообщений, чат становится "прочитанным" (не загорается оранжевым).
...
❌ <i>POC</i> не получает полную историю чата, вследствие чего видит только последнее сообщение, а не все. Если пользователь быстро напишет несколько сообщений, <i>POC</i> увидит только последнее.
❌ <i>POC</i> не может видеть изображения в чате и пересылать их в <i>Telegram</i> чат.
❌ <i>POC</i> не может точно определить, кто именно написал: ты или собеседник. Если чат не прочитан, значит сообщение от собеседника, иначе - от тебя. Однако, если ты смотришь чат в момент получения сообщений, данная логика иногда может сбоить. Так же <i>POC</i> не сможет определить, что в чат написало 3е лицо (арбитраж).

❗ Если нажать кнопку <code>Больше</code> в уведомлении о новом сообщении, <i>POC</i> "прочитает" чат и покажет последние 15 сообщений, в том числе изображения. Так же <i>POC</i> сможет определить, кто именно автор сообщений."""
...
❌ <i>Playerok Cardinal</i> еще <b><u>не инициализирован</u></b> и <b><u>никакие его функции не работают</u></b>.\n
🔃 Как только <i>Playerok Cardinal</i> инициализируется, данное сообщение изменится.\n
📋 Если <i>POC</i> долго не инициализируется, проверь логи с помощью команды /logs"""
...
update_lasted = "✅ У тебя стоит последняя версия Playerok Cardinal {}"
...
update_done = "✅ Установил обновление! Перезапусти <i>POC</i> с помощью команды /restart"
update_done_exe = "✅ Установил обновление! Новый <code>POC.exe</code> находится в папке <code>update</code>. " \
                  "Выключи <i>POC</i>, перенеси новый <code>POC.exe</code> на место старого " \
...
act_change_token = "Введите token"
token_changed = "✅ token успешно изменен{}.\n"
token_changed2 = "Перезапустите бота командой /restart."
token_incorrect_format = "❌ Неверный формат token. Попробуй ещё раз."
token_error = "❌ Не удалось авторизоваться. Возможно, введен неверный token?"
...
desc_lang = desc_main
desc_gs = "Здесь ты можешь включать и выключать основные функции <i>POC</i>."
...
upd_no_tags_api = "Не удалось получить теги с GitHub API"
upd_current_is_latest = "Текущий тег {} является последним"
upd_next_tag_found = "Следующий тег: {}"
upd_no_releases_data = "Не удалось получить данные о релизах"
